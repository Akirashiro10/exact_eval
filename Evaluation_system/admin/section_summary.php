    ...
    <select name="section" id="section" onchange="document.getElementById('sectionForm').submit()">
      ...
      if ($section_result && mysqli_num_rows($section_result) > 0) {
          while ($row = mysqli_fetch_assoc($section_result)) {
              ...
              $course_name = explode('-', $course_section)[0];
...
if ($selected_section === 'all') {
    ...
    if ($section_summary_result && mysqli_num_rows($section_summary_result) > 0) {
        ...
        echo "<table border='1'>";
        echo "<thead><tr><th>Department & Section</th><th>Total Students Evaluated</th><th>Total Enrolled</th><th>Completion Percentage</th><th>Average Score</th></tr></thead>";
        echo "<tbody>";
...
";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            echo "<h3>Total Students Evaluated : $total_evaluations / $total_students (". round($total_evaluations / $total_students * 100, 2) ."%)<br> For Department & Section: $course_name - $selected_section</h3><h3>Total Average Score: $average_score%</h3>";
        }
