...
$semester = $_GET['semester'] ?? null;
$facultyID = $_GET['FacultyID'] ?? null;
$course_query = "SELECT DISTINCT student_COURSE FROM student_info WHERE student_SECTION = ?";
        $course_stmt = mysqli_prepare($connection, $course_query);
        mysqli_stmt_bind_param($course_stmt, "s", $selected_section);
        mysqli_stmt_execute($course_stmt);
        $course_result = mysqli_stmt_get_result($course_stmt);
        $course_name = '';
        if ($course_result && mysqli_num_rows($course_result) > 0) {
            $course_row = mysqli_fetch_assoc($course_result);
            $course_name = htmlspecialchars($course_row['student_COURSE']);
        }
if ($selected_section === 'all') {
 $section_summary_query = "
...
SELECT 
 DISTINCT CONCAT(student_COURSE, ' - ', student_SECTION) AS course_section,
...
";
     ...
     if ($result && mysqli_num_rows($result) > 0) {
         ...
         echo "<p style='text-align:right;margin-top:-89px'><strong>Total Students Evaluated :</strong> $total_evaluations / $total_students (". round($total_evaluations / $total_students * 100, 2) ."%)</p><p style='text-align: right'><strong> For Department & Section: </strong>$course_name - $selected_section</p><h4>Total Average Score: $average_score%</h4><br>";
     }
     ...
     while ($criteria_row = mysqli_fetch_assoc($criteria_result)) {
         ...
         echo "<p><strong>Criteria:</strong> $criteria_name</p>";
