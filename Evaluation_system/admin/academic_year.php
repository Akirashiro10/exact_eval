...
<body>
  ...
  <div class="content">
    ...
    if ($selected_academic_year_result && mysqli_num_rows($selected_academic_year_result) > 0) {
      ...
      echo "<div class='box-header'>";
      echo "<h3>No current academic year selected.</h3>&nbsp;<h3>Please select anything from the list</h3>";
      echo "</div>";
    }
    ...
    if ($academic_year_result) {
      ...
      while ($row = mysqli_fetch_assoc($academic_year_result)) {
        ...
        if ($row['default_select'] == 1) {
          echo "<button class='default-button default-yes' id='default-{$row['id']}' onclick='toggleDefaultAcademicYear({$row['id']})'>Active</button>";
        } else {
          echo "<button class='default-button default-no' id='default-{$row['id']}' onclick='toggleDefaultAcademicYear({$row['id']})'>Inactive</button>";
        }
        echo "</td>";
        ...
        echo "<td>";
        if ($row['status'] != 0) {
          echo "<button class='finish-button' onclick='finishAcademicYear({$row['id']})'><i class='fas fa-flag-checkered'></i></button>&nbsp;&nbsp;";
        }
  ...
  <script>
    ...
    function finishAcademicYear(id) {
      if (confirm("Are you sure you want to mark this academic year as finished?")) {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "finish_academic_year.php", true);
