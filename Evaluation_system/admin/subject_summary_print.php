...
<?php
        ...
        $doc_num_query = "SELECT * FROM doc_name";
$stmt2 = mysqli_prepare($connection, $doc_num_query);
mysqli_stmt_execute($stmt2);

$doc_info = mysqli_stmt_get_result($stmt2);
$docnum = mysqli_fetch_assoc($doc_info);
  ...
  <h4 class="doccode" style="text-align:right; font-size:px; color:blue;"><?php echo $docnum['name']; echo $docnum['month']; echo $docnum['year'];?></h4>

            <h3 class="eval-report" style="text-align: center; text-decoration:underline">SUBJECT SUMMARY REPORT</h3>

            <h4 style="text-align:center;"> Academic Year: &nbsp;<?php echo htmlspecialchars($academicYear);?>, <?php echo htmlspecialchars($semester2); ?> Semester</h3>
            <h4> Faculty: <?php echo htmlspecialchars($faculty['Name']); ?></h3>
            <h4 style="text-align:right;margin-top:-41px">Total Students Evaluated:&nbsp;&nbsp; <?php echo $student_count; ?> / <?php echo $total_count; ?> (<?php echo round($student_count / $total_count * 100, 2); ?>%)</h4>

            <h4 style="text-align:left">Evaluation Results for: <?php echo ($selected_subject === 'all') ? 'All Subjects' : htmlspecialchars($selected_subject); ?></h4>
            <?php if ($selected_subject !== 'all'): ?>
                ...
                <div class="overall-rating">
                    <h4 style="text-align:right;margin-top:-41px;">Subject Total Score: <?php echo round($subject_avg_score, 2); ?>% (<?php echo $subject_performance; ?>)</h4>
                </div>
                <br>
            <?php endif; ?>
            <?php if ($selected_subject === 'all'): ?>
                <div class="overall-rating">
                    <h4 style="text-align:right;margin-top:-41px;">Overall Subject Score: <?php echo round($total_avg_rating, 2); ?>% (<?php echo $performance; ?>)</h4>
                </div>
            <?php endif; ?>
            <table border='1'>
                <thead>
